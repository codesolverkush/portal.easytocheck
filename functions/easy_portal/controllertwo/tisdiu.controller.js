...
//     // Verify signature
//     const expectedSignature = CryptoJS.HmacSHA256(data, hmacKey).toString();
//     if (expectedSignature !== signature) {
//       return res.status(403).json({ success: false, message: 'Invalid signature. Unauthorized request.' });
//     }
...
//     } catch (err) {
//       return res.status(400).json({ success: false, message: 'Failed to decrypt data.' });
//     }

//     const { orgid, email, timestamp } = JSON.parse(decryptedText);

//     // Optional anti-replay: Check timestamp freshness (eg. 5 mins)
//     const now = Date.now();
//     if (Math.abs(now - timestamp) > 300000) {
//       return res.status(408).json({ success: false, message: 'Request timed out or replay attack detected.' });
//     }

//     // ✅ Now your existing business logic
//     const { catalyst } = res.locals;
//     const zcql = catalyst.zcql();

//     if (!orgid || !email) {
//       return res.status(404).json({ success: false, message: 'Organization ID or Email missing.' });
//     }
...
//       return res.status(404).json({ success: false, message: 'No organization details found.' });
//     }

//     if (orgDetails[0]?.Organization?.superadminEmail === email) {
//       const selectUserQuery = `
//         SELECT * FROM usermanagement WHERE orgid = '${orgid}'
//       `;
...
//         // Validate HMAC
//         const computedSignature = CryptoJS.HmacSHA256(data, hmacKey).toString();
...
//         const { orgid, email, timestamp } = decryptedData;
//         console.log("Decrypted data:", decryptedData);



//         console.log("Decrypted crmorgid:", orgid);
...
//             return res.status(400).json({
//                 success: false,
//                 message: "Missing crmorgid or email in secure payload",
//             });
//         }

//         // Example replay protection (optional but recommended)
//         const now = Date.now();
...
//         // Query using decrypted crmorgid
//         const query = `SELECT * FROM Organization WHERE ROWID = '${orgid}'`;
//         const dataResult = await zcql.executeZCQLQuery(query);
