  ...
  const ToggleChangeDropdown = ({
    ...
    return (
      <select 
       ...
       >
        {
          Object.entries(TOGGLE_CHANGE).map(([key,level])=>(
           <option key={key} value={key}>
            {level}
           </option>
          ))
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        const userAccessInfo = users.map(user => ({
          ...
          changesDone: user.usermanagement.changesDone
        }));
          ...
          <table className="min-w-full bg-white border border-gray-200 rounded-lg">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className='py-3 px-4 text-center text-xs font-medium text-gray-500 uppercase tracking-wider'>Changes Done</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {isLoading ? (
                ...
                userAccessData.map((user) => (
                  <tr key={user.userid} className="hover:bg-gray-50">
                    ...
                    <td className='py-3 px-4'>
                        <ToggleChangeDropdown
                          userId={user.userid}
                          section="changesDone"
                          currentToggle={user.changesDone}
                        />
                    </td>
                  </tr>
                ))
