...
const OrganizationPersonalizationPanel = () => {
  ...
  const saveOrganizationDetails = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${process.env.REACT_APP_APP_API}/org/editorgdetails`, orgDetails);
...
const [isShowingSuggestions, setIsShowingSuggestions] = useState(false);
  ...
  const handleRemoveUser = async (id,useremail) => {
    ...
    try {
        ...
        `${process.env.REACT_APP_APP_API}/admin/removeuser/${id}`);      
      if(response.status === 200){
...
const searchContactByEmail = async () => {
  ...
  try {
    const response = await axios.get(`${process.env.REACT_APP_APP_API}/admin/searchcontact`, {
      params: {
...
//     try {
//       const response = await axios.post(`${process.env.REACT_APP_APP_API}/crm/clone-profile/${"5962890000000026014"}`,{
//         profileData
//       });
//       if(response.status === 200){
...
//       }
//     } catch (error) {
//       toast.error("Error comes during creation of new profile!");
//     } finally{
//       setIsSubmitting(false);
//     }
