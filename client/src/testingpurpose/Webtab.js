...
import { FaUserPlus, FaTrash, FaUsers, FaLock, FaExclamationTriangle, FaEdit, FaHome } from 'react-icons/fa';
...
const Webtab = () => {
  ...
  // New states for step control and loading
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Refresh overall data to update license counts and user list
  ...
  // Update contactDataSearch
  const contactDataSearch = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (response.status === 200 && response.data && response.data.data !== "") {
        ...
        setFormData({
          ...formData,
          first_name: response.data?.data?.data[0]?.First_Name || "",
          last_name: response.data?.data?.data[0]?.Last_Name || "",
          company: response.data?.data?.data[0]?.Account_Name.name || "",
          id: response.data?.data?.data[0]?.id || null,
          accountid: response.data?.data?.data[0]?.Account_Name?.id || null,
          // If you want to update email as well, uncomment below:
          // email_id: response.data.email || formData.email_id
        });
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto bg-white rounded-lg shadow-lg">
      ...
      <button className={`flex items-center space-x-2 text-white px-4 py-2 rounded-lg ${bgColors.primary} ${hoverColors.primary}`} onClick={() => navigate("/app/home")}>
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        <button
          ...
          className={`px-4 py-2 ${bgColors.primary} text-white rounded-md ${hoverColors.primary} transition-colors flex items-center`}
...
<button onClick={contactDataSearch}>Search</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div
            ...
            onClick={() => {
              ...
              setFormData({ email_id: "", first_name: "", last_name: "" ,company:""});
            }}
          ></div>
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden transform transition-all">
            ...
            <div className="px-6 py-5">
              <form onSubmit={showUserForm ? handleSubmit : contactDataSearch}>
                <div className="space-y-4">
                  ...
                  {/* Show these fields only after successful search */}
                  {showUserForm && (
                    <>
                      <div>
                ...
                {/* Show Add User button only if showUserForm is true */}
                {showUserForm && (
                  <div className="mt-6">
                    <button
                      type="submit"
