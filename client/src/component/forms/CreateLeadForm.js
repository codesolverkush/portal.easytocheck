...
//       let fieldData = response?.data?.data?.fields || [];
//       fieldData = fieldData.filter((field) => field.view_type?.create !== false);
...
//         `${process.env.REACT_APP_APP_API}/lead/createLead`,
//         formattedData
//       );
  ...
  async function fetchCRMFields() {
    try {
      ...
      if (cachedResponse) {
        ...
        console.log("Lead form fields loaded from Cache Storage!");
        return;
      }
      ...
      const response = await axios.get(
        ...
        console.log("getlead api", response);
      ...
      console.log("Lead form fields fetched from API and cached!");
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        try {
          ...
          console.log("Leads cache cleared after creating new lead");
        } catch (cacheError) {
