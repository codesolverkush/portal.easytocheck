  ...
  return (
    <div className="animate-pulse">
      ...
      <div className="p-4">
        ...
        {[...Array(5)].map((_, index) => (
          <div key={index} className="h-32 sm:h-20 bg-gray-200 my-2 rounded w-full"></div>
        ))}
      </div>
    </div>
  );
...
const LeadView = () => {
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_APP_API}/get/leaddetails`);
      ...
      if (response.status === 200) {
        ...
        const newResponse = new Response(JSON.stringify(response?.data), { headers: { "Content-Type": "application/json" } });
      ...
      <div className="p-4 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div className="flex flex-col sm:flex-row w-full justify-between mb-4 md:mb-0 mr-5">
            <h1 className="text-2xl font-bold text-gray-800 mb-3 sm:mb-0">Lead Management</h1>
            <div className="flex flex-wrap gap-2 sm:gap-4">
              <button onClick={hardSync} className="flex-1 sm:flex-none bg-blue-500 text-white px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center hover:bg-blue-600 transition-colors text-sm md:text-base">
        ...
        <div className="md:hidden relative mb-4">
          <div className="flex items-center justify-between bg-white p-3 rounded-lg shadow">
            ...
            <div className="text-sm text-gray-600">
              {totalLeads > 0 
                ? `${indexOfFirstLead + 1}-${Math.min(indexOfLastLead, totalLeads)} of ${totalLeads}`
                : `0 contacts`}
            </div>
          </div>
            ...
            <div className="absolute left-0 right-0 mt-1 bg-white border rounded-lg shadow-lg z-10">
              {[10, 20, 30, 50, 100].map((value) => (
                <button
                  ...
                  className={`block w-full text-left px-4 py-2 text-sm ${leadsPerPage === value ? 'bg-blue-50 text-blue-600' : 'hover:bg-gray-50'}`}
                >
                  {value}
                </button>
              ))}
        ...
        <div className="hidden md:flex justify-between items-center mb-4 bg-white p-3 rounded-lg shadow">
          <div className="flex items-center">
            <label htmlFor="leadsPerPage" className="mr-2 text-sm text-gray-600">Show:</label>
            <select
              id="leadsPerPage"
              value={leadsPerPage}
          ...
          <div className="flex items-center">
            <span className="text-sm text-gray-600 mr-4">
              {totalLeads > 0 
                ? `Showing ${indexOfFirstLead + 1} to ${Math.min(indexOfLastLead, totalLeads)} of ${totalLeads} leads`
                : `0 leads found`}
            </span>
            <div className="flex">
              <button
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                <th className="p-4 cursor-pointer" onClick={() => handleSort("Last_Name")}>
                  <div className="flex items-center">
                    Lead Name
                    {sortField === "Last_Name" && (
                      <span className="ml-1">
                ...
                <th className="p-4 cursor-pointer" onClick={() => handleSort("Company")}>
                  <div className="flex items-center">
                    Company
                    {sortField === "Company" && (
                      <span className="ml-1">
            ...
            <tbody>
              {currentLeads.length > 0 ? (
                currentLeads.map((lead, index) => (
                  ...
                  >                   
                    <td className="p-4 text-sm font-medium text-gray-700">{lead.Full_Name || "-"}</td>
                    ...
                    <td className="p-4 text-sm">{moment(lead.Created_Time).format("DD-MM-YY HH:mm") || "-"}</td>
                  </tr>
                ))
              ) : (
                <tr>
            ...
            currentLeads.map((lead, index) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-medium text-gray-800">{lead.Last_Name || "Unnamed Lead"}</h3>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[lead.Lead_Status] || "bg-gray-200 text-gray-700"}`}>
                    {lead.Lead_Status || "New"}
                  </span>
                </div>
                <p className="text-sm text-gray-600 mb-1">{lead.Company || "No company"}</p>
                <div className="flex flex-wrap gap-2 mt-2">
                  {lead.Email && (
                    <div className="text-xs text-gray-500 flex items-center">
                      <span className="font-medium mr-1">Email:</span> {lead.Email}
                    </div>
                  )}
                  {lead.Phone && (
                    <div className="text-xs text-gray-500 flex items-center">
                      <span className="font-medium mr-1">Phone:</span> {lead.Phone}
                    </div>
                  )}
                </div>
                {lead.tag && (
          ...
          <div className="bg-white rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4">
              <div className="grid grid-cols-1 gap-4">
                <div>
                  <span className="block text-xs font-medium text-gray-500">LEAD NAME</span>
                  <span className="block text-sm">{selectedLead.Last_Name || "-"}</span>
                </div>
                <div>
                  <span className="block text-xs font-medium text-gray-500">COMPANY</span>
                  <span className="block text-sm">{selectedLead.Company || "-"}</span>
                </div>
                <div>
                  <span className="block text-xs font-medium text-gray-500">LEAD STATUS</span>
                  <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${statusColors[selectedLead.Lead_Status] || "bg-gray-200 text-gray-700"}`}>
                ...
                <div>
                  ...
                  <span className="block text-sm">{moment(selectedLead.Created_Time).format("DD-MM-YY HH:mm") || "-"}</span>
                </div>
                {selectedLead.tag && (
                  <div>
                    <span className="block text-xs font-medium text-gray-500">TAG</span>
                    <span className="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs flex items-center w-fit">
                      📅 {selectedLead.tag}
                    </span>
                  </div>
...
//   const getEachRecordHandler = (leadId) => {
//     navigate('/app/leadprofile', { state: { leadId } });
//   };
...
//                     onClick={() => getEachRecordHandler(lead.id)}
//                   >
//                     {/* <td className="p-4 text-sm">{lead.id}</td> */}
//                     <td className="p-4 text-sm">
//                       {lead.tag ? (
