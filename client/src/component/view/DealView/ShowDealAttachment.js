...
const ShowDealAttachement = ({ dealId, cachedData, setCachedData, dataLoaded }) => {
    ...
    const fetchAttachments = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_APP_API}/related/getattach/Deals/${dealId}`,
        ...
        if (response?.data?.success && Array.isArray(response?.data?.data?.data)) {
          ...
          const attachmentsWithDealId = fetchedAttachments.map(attachment => ({
            ...attachment,
            dealId: dealId
          })); 
  
          setAttachments(attachmentsWithDealId);
          if (setCachedData) setCachedData(attachmentsWithDealId);
        } else {
          setAttachments([]);
  ...
  const refreshAttachments = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_APP_API}/related/getattach/Deal/${dealId}`, {
        withCredentials: true
      });
...
export default ShowDealAttachement;