...
const ShowContactAttachement = ({ contactId, cachedData, setCachedData, dataLoaded }) => {
    ...
    const fetchAttachments = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_APP_API}/related/getattach/Contacts/${contactId}`,
        ...
        if (response?.data?.success && Array.isArray(response?.data?.data?.data)) {
          ...
          const attachmentsWithContactId = fetchedAttachments.map(attachment => ({
            ...attachment,
            contactId: contactId
          })); 
  
          setAttachments(attachmentsWithContactId);
          if (setCachedData) setCachedData(attachmentsWithContactId);
        } else {
          setAttachments([]);
  ...
  const refreshAttachments = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_APP_API}/related/getattach/Contacts/${contactId}`, {
        withCredentials: true
      });
...
export default ShowContactAttachement;