...
const ContactView = () => {
  ...
  const saveColumnSettings = () => {
    ...
    localStorage.setItem("crm-contact-columns", JSON.stringify(selectedColumns));
    setShowColumnModal(false);
  };

  // Add this useEffect to load saved column preferences on component mount
  useEffect(() => {
    const savedColumns = localStorage.getItem("crm-contact-columns");
    if (savedColumns) {
    ...
    if (searchTerm) {
      ...
      filteredContacts = filteredContacts.filter(
        (contact) =>
          ...
          (contact.Lead_Source &&
            contact.Lead_Source.toLowerCase().includes(term))
      );
    }
              ...
              <div className="flex flex-wrap gap-2 sm:gap-4">
                ...
                {accessScore < 2 ? (
                  <Link
                    ...
                    onClick={() =>
                      toast.error(
                        "Insufficient access rights to create a contact"
                      )
                    }
          ...
          <div className="md:hidden relative mb-4">
            <div className="flex items-center justify-between bg-white p-3 rounded-lg shadow">
              ...
              <div className="text-sm text-gray-600">
                {totalContacts > 0
                  ? `${indexOfFirstContact + 1}-${Math.min(
                      indexOfLastContact,
                      totalContacts
                    )} of ${totalContacts}`
                  : `0 contacts`}
          ...
          <div className="hidden md:flex justify-between items-center mb-4 bg-white p-3 rounded-lg shadow">
            ...
            <div className="flex items-center">
              <span className="text-sm text-gray-600 mr-4">
                {totalContacts > 0
                  ? `Showing ${indexOfFirstContact + 1} to ${Math.min(
                      indexOfLastContact,
                      totalContacts
              ...
              <table className="w-full">
                ...
                <tbody>
                  {currentContacts.length > 0 ? (
                    currentContacts.map((lead, index) => (
                      <tr
                        key={index}
                ...
                currentContacts.map((contact) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${
                          sourceColors[contact.Lead_Source] ||
                          "bg-gray-200 text-gray-700"
                        }`}
                      >
                        {contact.Lead_Source || "-"}
                  ...
                  <div>
                    ...
                    <span
                      className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
                        sourceColors[selectedContact.Lead_Source] ||
                        "bg-gray-200 text-gray-700"
                      }`}
                    >
