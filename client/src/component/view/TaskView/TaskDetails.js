...
const [editingTask, setEditingTask] = useState({
  ...
  // Reset visibleCount if taskList changes (e.g., after create/delete)
  useEffect(() => {
    setVisibleCount(20);
  }, [taskList]);
    ...
    <div className="flex flex-col lg:flex-row h-[calc(100vh-64px)]">
      ...
      <div className="hidden lg:flex lg:w-1/3 xl:w-1/4 flex-col border-r border-gray-200 bg-white">
        ...
        <div className="overflow-y-auto flex-1">
          {taskList?.data?.slice(0, visibleCount).map((task) => (
            <div
              key={task.id}
              onClick={() => {
          ...
          {/* Show More Button */}
          {taskList?.data && visibleCount < taskList.data.length && (
            <div className="flex justify-center py-4">
              <button
                onClick={() => setVisibleCount(visibleCount + 20)}
                className={`px-4 py-2 ${bgColors.primary} text-white rounded ${hoverColors.primary} border border-gray-300`}
              >
                Show More
              </button>
            </div>
              ...
              <div className="bg-white rounded-lg shadow-lg">
                <div className="border-b p-6">
                  <div className="flex justify-between items-center">
                    <Link
                      to="#"
                      className={`${textColors.primary} ${hoverColors.textPrimary} text-xl font-semibold`}
