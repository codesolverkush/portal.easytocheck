      ...
      <div className="p-4 max-w-7xl mx-auto">
        ...
        <div className="md:hidden relative mb-4">
          <div className="flex items-center justify-between bg-white p-3 rounded-lg shadow">
            ...
            <div className="text-sm text-gray-600">
              {totalTasks > 0
                ? `${indexOfFirstTask + 1}-${Math.min(indexOfLastTask, totalTasks)} of ${totalTasks}`
                : `0 tasks`}
            </div>
          </div>
            ...
            <div className="absolute left-0 right-0 mt-1 bg-white border rounded-lg shadow-lg z-10">
              {[10, 20, 30, 50, 100].map((value) => (
                <button
                  ...
                  className={`block w-full text-left px-4 py-2 text-sm ${tasksPerPage === value ? 'bg-blue-50 text-blue-600' : 'hover:bg-gray-50'}`}
                >
                  {value}
                </button>
              ))}
        ...
        <div className="hidden md:flex justify-between items-center mb-4 bg-white p-2 rounded-lg shadow">
          <div className="flex items-center mb-2 sm:mb-0">
            <label htmlFor="tasksPerPage" className="mr-2 text-sm text-gray-600">Show:</label>
            <select
              id="tasksPerPage"
              value={tasksPerPage}
              ...
              {currentTasks.length > 0 ? (
                currentTasks.map((task, index) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm">{moment(task.Created_Time).format("DD-MM-YY HH:mm") || "-"}</td>
                  </tr>
                ))
              ) : (
                <tr>
